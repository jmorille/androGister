...
public class OfferProvider extends ContentProvider {
	...
	/**
	 * Handles all the dictionary searches and suggestion queries from the Search Manager. When requesting a specific word, the uri alone is required. When
	 * searching all of the dictionary for matches, the selectionArgs argument must carry the search query as the first element. All other arguments are
	 * ignored.
	 */
	@Override
...
//		String[] columns = new String[] { ProductDatabase.Column.KEY_ID, ProductDatabase.Column.KEY_NAME, ProductDatabase.Column.KEY_DESCRIPTION };
	...
	private Cursor refreshShortcut(Uri uri) {
		/*
		 * This won't be called with the current implementation, but if we include {@link SearchManager#SUGGEST_COLUMN_SHORTCUT_ID} as a column in our
		 * suggestions table, we could expect to receive refresh queries when a shortcutted suggestion is displayed in Quick Search Box. In which case, this
		 * method will query the table for the specific word, using the given item Uri and provide all the columns originally provided with the suggestion
		 * query.
		 */
		String rowId = uri.getLastPathSegment();
		String[] columns = new String[] { OfferDatabase.Column.KEY_ID, OfferDatabase.Column.KEY_NAME, OfferDatabase.Column.KEY_DESCRIPTION,
